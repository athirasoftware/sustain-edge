...
<form action="javascript:void(0)">
    <div class="row">
        <div class="col-lg-6">
            ...
            <select name="stationaryParticular" id="stationaryParticular" class="form-control Particular">
                ...
                @if (isset($stationaryParticulars) && count($stationaryParticulars) > 0)
                    @foreach ($stationaryParticulars as $key => $stationaryParticular)
                        <option
                            value="{{ $stationaryParticular['id'] }}+{{ $stationaryParticular['particulars'] }}+{{ $stationaryParticular['quantity_type'] }}">
                            {{ $stationaryParticular['particulars'] }}</option>
                    @endforeach
                @endif
            </select>
...
<div>
    <div class="cmnBx mt-4 formDivClass" id="formDiv" style="display:none">
        <div class="row cntr">
            <div class="col-lg-4">
            ...
            <div class="col-lg-4">
                <input type="hidden" class="fuelUOM" name="fuelUOM" id="fuelUOM" value="" />
                <input type="hidden" class="randomId" name="randomId" id="randomId" value="" />
                <label for="">Region*</label>
                <select name="region" id="region" class="form-control region_type">
            ...
            <div class="col-lg-4">
                ...
                <select name="unitOfMesurement" id="unitOfMesurement" class="form-control Region">
                ...
                <div class="rtsertBxc">
                    <div class="item">
                        <a href="javascript:void(0)" class="cmnbtn hoveranim removeForm"
                            id="removeForm"><span>Cancel</span> <span class="randomNum" style="display:none"></span></a>
                    </div>
                    <div class="item">
    ...
    @if (isset($existingStationaryParticulars) && count($existingStationaryParticulars) > 0)
        @foreach ($existingStationaryParticulars as $key => $existingStationaryParticular)
            ...
            <div class="item">
                <div class="itemBx">
                    <div class="left">
                        <div class="txtT"><b>Fuel:</b> <span
                                id="editStationaryParticularText_{{ $randId }}">{{ $existingStationaryParticular->fuel_particular }}</span>

                            <label style="margin-left:15px;"> Total Emission:
                                {{ $existingStationaryParticular->total_emission }}</label> <span>
                    ...
                    <div class="rtsertBxc" id="selectedView_{{ $randId }}">
                        <div class="item">
                            <button
                                onclick="editView('{{ $randId }}', '{{ Crypt::encrypt($existingStationaryParticular->id) }}');"
                                class="cmnbtn hoveranim"><span>Edit</span></button>
                        </div>
                        <div class="item">
                            <button
                                onclick="deleteStationaryCombustion('{{ Crypt::encrypt($existingStationaryParticular->id) }}');"
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('.container').on('click', '.region_type', function() {
            var stationaryParticularVal = $('#stationaryParticular').val().split("+");
            var randomId = $(".randomId").val();
            // console.log(" fuelTypeId => "+fuelTypeId+" fuelTypeVal => "+fuelTypeVal);
            uomList(stationaryParticularVal[2],randomId);
        });
    });
    ...
    function uomList(stationaryParticularVal,randomId) {
        $.ajax({
            ...
            async: false,
            data: {
                "selectedId": stationaryParticularVal,
                "_token": "{{ csrf_token() }}",
            },
            success: function(data) {
                ...
                var $el = $("#unitOfMesurement_" + randomId);
                var items = "";
                $el.html("<option value=''>Select Unit of measurement</option>");
    ...
    // function fueltype_uom(fuel_UOM) {
    //     console.log(fuel_UOM);
    //     var fuelTypeVal = fuel_UOM;


    //         var $el = $("#unitOfMesurement");

    //        if(fuelTypeVal=="solid"){
    //         items += "<option>kg</option>";
    //         $el.append(items);
    //        }
    // }
</script>
