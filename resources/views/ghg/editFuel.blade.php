<div class="cmnBx mt-4">
    <div class="row cntr">
        ...
        <div class="col-lg-4">
            <input type="hidden" name="fuelUOM_{{ $randomId }}" id="fuelUOM"
                value="{{ $selfuelTypes->quantity_type }}" />
            ...
            <select name="region_{{ $randomId }}" id="region_{{ $randomId }}" class="form-control">
                ...
                @if (isset($regionArray) && count($regionArray) > 0)
                    @foreach ($regionArray as $id => $region)
                        <option value="{{ $id }}" @if (isset($existingStationaryParticulars->region) && $id == $existingStationaryParticulars->region) selected @endif>
                            {{ $region }}</option>
                    @endforeach
                @endif
            </select>
                ...
                @if (isset($fuelTypes) && count($fuelTypes) > 0)
                    @foreach ($fuelTypes as $id => $fuelType)
                        @if (isset($fuelType) && $fuelType != '')
                            <option value="{{ $id }}" @if (isset($existingStationaryParticulars->input_uom) && $id == $existingStationaryParticulars->input_uom) selected @endif>
                                {{ $fuelType }}</option>
                        @endif
                    @endforeach
                @endif
        ...
        <input type="hidden" name="combustionId_{{ $randomId }}" id="combustionId_{{ $randomId }}"
            value="{{ Crypt::encrypt($combustionId) }}" />
        <input type="hidden" name="particularId_{{ $randomId }}" id="particularId_{{ $randomId }}"
            value="{{ Crypt::encrypt($existingStationaryParticulars->fuel_particular_id) }}" />
        <div class="col-lg-4">
            <label for="">Quantity (Actual)*</label>
            <input type="text" name="quantityActual_{{ $randomId }}" id="quantityActual_{{ $randomId }}"
                value="{{ $existingStationaryParticulars->actual_quantity }}" class="form-control">
        ...
        <div class="col-lg-6">
            <div class="rtsertBxc">
                <div class="item">
                    <a href="javascript:void(0)" class="cmnbtn hoveranim removeEdit"
                        id="removeForm_{{ $randomId }}"><span>Cancel</span> <span class="randomNum"
                            style="display:none"></span></a>
                </div>
                <div class="item">
                    <a href="javascript:void(0)" class="cmnbtn hoveranim stationaryUpdate"
                        id="stationaryUpdate_{{ $randomId }}"><span>Update</span><span class="randomNum"
                            style="display:none"></span></a>
                </div>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        $(".container").on('click', '.Region', function() {
            var stationaryParticularVal = $('#fuelUOM').val();
