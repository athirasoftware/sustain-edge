<div  id="dashBoarAccord">
    <div class="accordion-item">
        ...
        <div id="dashBoarAccord1">
            ...
            <div class="stationaryBox cmnBx show" id="mobileCombustionDiv">
                    @include('ghg.mobileCombustion')
                </div>
...
<script type="text/javascript">
    ...
    function mobileduplicateDivContent(original, sno) {
    ...
    function mobilecheckForAddBtn() {
        console.log("inside add btn mobile");
        var stationaryParticularVal = $('#mobileParticular').val();
        ...
        if(stationaryParticularValArray[0] != '' && typeof stationaryParticularValArray[0] !== 'undefined') {
            ...
            $('#mobilefuelUOM').val(stationaryParticularValArray[2]);
        } else {
    ...
    function mobilesaveFormCombustionForm(randomNum) {
       $('.displayMsg').html('');
        var particularId = $('#particularId').val();
        // var selectedFuel = $('#stationaryParticular').val();
        var selectedFuel = $( "#mobileParticular option:selected" ).text();
       ...
       // var fuelType = $("#fuelType_"+randomNum+" option:selected").text();
        ...
        if(selectedFuel != ''  && region != '' && unitOfMesurement != '' && quantityActual != '') {
            $.ajax({
                url: "{{ url('/mobilesaveStationaryCombution') }}",
                method: 'post',
                dataType:"JSON",
                data: {
                    "randomNum": randomNum,
                ...
                success: function(data) {
                    ...
                    if(data.status == 'success') {
                        ...
                        setTimeout(() => {
                            $('.displayMsg').html('');
                            $('.displayMsg').hide();
                            $('#mobileCombustionDiv').load("{{ url('/getRefreshMobileCombution')}}");
                        }, 2000);
    ...
    function deleteMobileCombustion(selectedFuelId) {
        // console.log(selectedFuelId);
            $.ajax({
                url: "{{ url('/deleteMobileCombution') }}",
                method: 'post',
                dataType:"JSON",
                data: {
    ...
    function updateFormMobileCombustionForm(randomNum) {
        $('.updateMsg').html('');
       ...
       // var fuelType = $("#fuelType_"+randomNum+" option:selected").text();
        ...
        if(selectedFuel != ''  && region != '' && unitOfMesurement != '' && quantityActual != '') {
            $.ajax({
                url: "{{ url('/updateMobileCombustionInfo') }}",
                method: 'post',
                dataType:"JSON",
                data: {
                    "randomNum": randomNum,
