...
<form action="javascript:void(0)">
    <div class="row">
                ...
                @if(isset($stationaryParticulars) && count($stationaryParticulars) > 0) 
                    @foreach($stationaryParticulars as $key => $stationaryParticular)
                        @if(isset($stationaryParticular) && $stationaryParticular != '')
                            <option value="{{$key}}+{{$stationaryParticular}}">{{$stationaryParticular}}</option>
                        @endif
                    @endforeach
                @endif
            </select>
        ...
        <div class="col-lg-6 mt-4 floatLeft">
                <a href="javascript:void(0);" class="addNewForm" title="Add Form" style="text-decoration:none">ADD NEW</i></a>
            </div>
    </div>
</form>
    ...
    @if(isset($existingStationaryParticulars) && count($existingStationaryParticulars) > 0)
            ...
            <?php $randId = rand(100,100000); ?>
                <div class="item">
                    <div class="itemBx">
                        <div class="left">
                        <div class="txtT"><b>Fuel:</b> <span id="editStationaryParticularText_{{$randId}}">{{$existingStationaryParticular->fuel_particular}} {{$existingStationaryParticular->converted_uom}}</span>
                            <span id="">
                            <label style="margin-left:15px;"><b>Total Emission:</b></label> {{$existingStationaryParticular->total_emission}} {{$existingStationaryParticular->standard}}</span> 
                ...
                <div class="rtsertBxc">
                    <div class="item">
                        <a href="javascript:void(0)"
                            class="cmnbtn hoveranim removeForm"  id="removeForm"><span>Cancel</span> <span class="randomNum" style="display:none"></span></a>
                    </div>
                    <div class="item">
...
<script type="text/javascript">
    ...
    function editView(randomId, combustionId) {
        var url = "{{URL::TO('editFuelPurchaseofElectricity')}}";
